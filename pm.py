...
def main():
	if args.option in ["add","a"]:
		if args.name == None or args.url == None or args.login == None:
			if args.name == None:
				printc("Site Name (-s) required ")
			if args.url == None:
				printc(" Site URL (-u) required ")
			if args.login == None:
				printc("Site Login (-l) required ")
			return
	...
	if args.option in ["generate","g"]:
		if args.length == None:
			printc("Specify length of the password to generate (--length)")
			return
		password = utils.generate.generatePassword(args.length)
		pyperclip.copy(password)
		printc("Password generated and copied to clipboard")
